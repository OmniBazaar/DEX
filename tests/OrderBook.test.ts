...
describe("OrderBook", function () {
  ...
  beforeEach(async function () {
    ...
    await tokenA.connect(maker).approve(orderBookAddress, ethers.MaxUint256);
    await tokenB.connect(maker).approve(orderBookAddress, ethers.MaxUint256);
    await tokenA.connect(taker).approve(orderBookAddress, ethers.MaxUint256);
    await tokenB.connect(taker).approve(orderBookAddress, ethers.MaxUint256);
  });

  describe("Order Placement", function () {
    it("Should place a buy limit order", async function () {
      const price = ethers.parseEther("2000"); // 1 TokenA = 2000 TokenB
      ...
      expect(order.status).to.equal(1); // OrderStatus.Open
    });

    it("Should place a sell limit order", async function () {
      const price = ethers.parseEther("2100");
  ...
  describe("Order Matching", function () {
    it("Should match buy and sell orders at same price", async function () {
      ...
      expect(buyOrder.status).to.equal(2); // OrderStatus.Filled
      expect(sellOrder.filled).to.equal(amount);
      expect(sellOrder.status).to.equal(2); // OrderStatus.Filled
    });
    ...
    it("Should partially fill orders", async function () {
      ...
      expect(buyOrder.status).to.equal(1); // OrderStatus.Open
    });

    it("Should match orders with price improvement", async function () {
      const buyPrice = ethers.parseEther("2100");
      const sellPrice = ethers.parseEther("2000");
      const amount = ethers.parseEther("1");
  ...
  describe("Market Orders", function () {
    beforeEach(async function () {
      ...
      // Buy orders
      await orderBook.connect(maker).placeLimitOrder(true, basePrice - ethers.parseEther("20"), amount);
      await orderBook.connect(maker).placeLimitOrder(true, basePrice - ethers.parseEther("10"), amount);
      await orderBook.connect(maker).placeLimitOrder(true, basePrice, amount);

      // Sell orders
      await orderBook.connect(maker).placeLimitOrder(false, basePrice + ethers.parseEther("10"), amount);
      await orderBook.connect(maker).placeLimitOrder(false, basePrice + ethers.parseEther("20"), amount);
    });
  ...
  describe("Fee Collection", function () {
    it("Should collect correct maker and taker fees", async function () {
      ...
      const tradeValue = price * amount / ethers.parseEther("1");
      const makerFeeAmount = tradeValue * BigInt(MAKER_FEE) / BigInt(FEE_DENOMINATOR);
      const takerFeeAmount = tradeValue * BigInt(TAKER_FEE) / BigInt(FEE_DENOMINATOR);
      const totalFees = makerFeeAmount + takerFeeAmount;

      const finalFeeBalance = await tokenB.balanceOf(feeCollector.address);
      expect(finalFeeBalance - initialFeeBalance).to.equal(totalFees);
    });
  });
  ...
  describe("Order Book Queries", function () {
    beforeEach(async function () {
      ...
      for (let i = 0; i < 5; i++) {
        await orderBook.connect(maker).placeLimitOrder(
          true,
          basePrice - ethers.parseEther((i * 10).toString()),
          amount
        );
        await orderBook.connect(maker).placeLimitOrder(
          false,
          basePrice + ethers.parseEther(((i + 1) * 10).toString()),
          amount
        );
      }
    });
    ...
    it("Should get best bid and ask", async function () {
      ...
      expect(bestBid.price).to.equal(ethers.parseEther("2000"));
      expect(bestAsk.price).to.equal(ethers.parseEther("2010"));
    });
  });

  describe("Stop Orders", function () {
    it("Should place stop-loss order", async function () {
      const stopPrice = ethers.parseEther("1900");
      const limitPrice = ethers.parseEther("1890");
      const amount = ethers.parseEther("1");
    ...
    it("Should trigger stop order when price crosses threshold", async function () {
      ...
      await orderBook.connect(taker).placeLimitOrder(true, ethers.parseEther("1895"), amount);
      await orderBook.connect(maker).placeLimitOrder(false, ethers.parseEther("1895"), amount);
